...
async def get_tools_result(
    ...
    if function_call_name == "get_search_results":
        ...
        llm = robot(
            api_key=api_key,
            api_url=api_url.source_api_url,
            engine=engine,
            use_plugins=use_plugins,
        )
        keywords = (
            await llm.ask(search_key_word_prompt.format(source=prompt), model=model)
        ).split("\n")
        ...
        function_response, text_len = cut_message(
    ...
    function_response = f"function_response:{function_response}"
